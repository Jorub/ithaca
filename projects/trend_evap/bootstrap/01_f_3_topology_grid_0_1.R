...
evap_trend[slope < 0 & p <= 0.1, neg_signal := 1]

### Dampener ----
evap_trend[, signal_dampener := 0]
evap_trend[p > 0.1, signal_dampener:= 1]

### merge ensemble stats ---
evap_trend_merge <- merge(evap_trend, 
                          evap_trend_summary[,.(lon, lat, DCI_0_1, N_sig_0_1, N_none_0_1, 
                                                N_pos_0_1, N_neg_0_1, trend_0_1)], 
                          by = c("lon", "lat"))

evap_trend_merge[, opposing_majority_trend := 0]
evap_trend_merge[DCI_0_1/slope < 0 & DCI_0_1 != 0, opposing_majority_trend := 1]
evap_trend_merge[, opposing_significance := 0]
evap_trend_merge[(N_sig_0_1 > N_none_0_1) & p > 0.1, opposing_significance := 1]
evap_trend_merge[(N_none_0_1 > N_sig_0_1) & p <= 0.1, opposing_significance := 1]
...
saveRDS(evap_trend_merge[,.(lon, lat, dataset, pos_signal, neg_signal, signal_dampener,
        ...
        paste0(PATH_SAVE_EVAP_TREND, "global_grid_cell_topology_0_1.rds"))
