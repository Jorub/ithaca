...
### Input data generated in trend_evap/bootstrap/04_b
evap_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "global_grid_per_dataset_evap_slope_intersection_lat_lon_bootstrap.rds"))  
evap_trend <- evap_trend[!dataset %in% c("gldas-vic", "gldas-noah")]

evap_trend_summary <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "global_grid_topology_input_pos_boosters_out.rds"))
...
saveRDS(evap_trend_merge[,.(lon, lat, dataset, pos_signal, neg_signal, signal_dampener,
        ...
        paste0(PATH_SAVE_EVAP_TREND, "global_grid_topology_pos_out.rds"))
