...
evap_trend[slope > 0 & p <= 0.01, pos_signal := 1]

### Negative trend booster ----
evap_trend[, neg_signal := 0]
evap_trend[slope < 0 & p <= 0.01, neg_signal := 1]

### Dampener ----
evap_trend[, signal_dampener := 0]
evap_trend[p > 0.01, signal_dampener:= 1]

### merge ensemble stats ---
evap_trend_merge <- merge(evap_trend, 
                          evap_trend_summary[,.(lon, lat, DCI_0_01, N_sig_0_01, N_none_0_01, 
                                                N_pos_0_01, N_neg_0_01, trend_0_01)], 
                          by = c("lon", "lat"))

evap_trend_merge[, opposing_majority_trend := 0]
evap_trend_merge[DCI_0_01/slope < 0 & DCI_0_01 != 0, opposing_majority_trend := 1]
evap_trend_merge[, opposing_significance := 0]
evap_trend_merge[(N_sig_0_01 > N_none_0_01) & p > 0.01, opposing_significance := 1]
evap_trend_merge[(N_none_0_01 > N_sig_0_01) & p <= 0.01, opposing_significance := 1]
...
saveRDS(evap_trend_merge[,.(lon, lat, dataset, pos_signal, neg_signal, signal_dampener,
        ...
        paste0(PATH_SAVE_EVAP_TREND, "global_grid_cell_topology_0_01.rds"))
