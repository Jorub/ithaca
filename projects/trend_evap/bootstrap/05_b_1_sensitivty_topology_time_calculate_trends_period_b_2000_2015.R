...
evap_datasets <- evap_datasets[year < 2015]
...
for (dataset_num in global_datasets){
  ...
  saveRDS(evap_dataset_sel_trend, 
          paste0(PATH_SAVE_EVAP_TREND, "evap_dataset_",dataset_num,"_trend_bootstrap_2000_2014.rds"))  
}

### Assemble data ----

for (dataset_num in global_datasets){
  dummy_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "evap_dataset_",dataset_num,"_trend_bootstrap_2000_2014.rds"))  
  if(dataset_num != global_datasets[1]){
    evap_trend <- rbind( evap_trend, dummy_trend)
  }else{
    evap_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "evap_dataset_",dataset_num,"_trend_bootstrap_2000_2014.rds"))  
  }
}

### Merged data save
saveRDS(evap_trend, paste0(PATH_SAVE_EVAP_TREND, "global_grid_per_dataset_evap_slope_bootstrap_merged_2000_2014.rds"))  
...
## save data----
saveRDS(evap_trend, paste0(PATH_SAVE_EVAP_TREND, "global_grid_per_dataset_evap_slope_bootstrap_2000_2014.rds"))  
saveRDS(evap_trend_complete_only , paste0(PATH_SAVE_EVAP_TREND, "global_grid_per_dataset_evap_slope_intersection_lat_lon_bootstrap_2000_2014.rds"))  
