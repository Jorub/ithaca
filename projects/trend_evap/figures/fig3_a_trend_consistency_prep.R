...
evap_trend_masks[, lat_brk := cut(lat, seq(-60, 90, 15))]

## Latitudinal groups  ----
data_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "lat_trend_bootstrap.rds"))  
ensemble_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "lat_groups_ensemble_trend_bootstrap.rds"))  

ensemble_trend[, dataset := "ensemble"]

data_trend <- merge(data_trend, ensemble_trend, 
                    by = c("dataset", "lat_brk", "p", "slope", "lower", "upper"),  all = T)
...
data_trend[dataset == "GLDAS-NOAH", dataset := "GLDAS-Noah"]
...
data_trend_env <- data_trend[dataset != "ensemble",.(Q25 = quantile(slope, 0.25),
                                                     Q75 = quantile(slope, 0.75)), lat_brk]
...
### problems  ----
lat_trends <- evap_trend_masks[,.(problem_area = sum(area)),.(problem, lat_brk)]
lat_trends <- lat_trends[complete.cases(lat_brk)]
lat_trends[, lat_area:= sum(problem_area), .(lat_brk)]
lat_trends[, lat_fraction:= problem_area/lat_area]
lat_trends <- lat_trends[!is.na(lat_brk)]

### save data ----
saveRDS(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_lat_groups_trends_by_product.rds"))
saveRDS(lat_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_lat_groups_problem_area_fraction.rds"))
saveRDS(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_lat_groups_problem_aggregated.rds"))
write.csv(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_lat_groups_trends_by_product.csv"))
write.csv(lat_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_lat_groups_problem_area_fraction.csv"))
write.csv(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_lat_groups_problem_aggregated.csv"))
...
### save data ----
saveRDS(data_trend[biome_short_class != "N/A"], paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_biome_trends_by_product.rds"))
saveRDS(biome_trends[biome_short_class != "N/A"], paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_biome_problem_area_fraction.rds"))
saveRDS(data_trend_env[biome_short_class != "N/A"], paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_biome_problem_aggregated.rds"))
...
### save data ----
saveRDS(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_IPCC_ref_regions_trends_by_product.rds"))
saveRDS(ipcc_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_IPCC_ref_regions_problem_area_fraction.rds"))
saveRDS(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_IPCC_ref_regions_problem_aggregated.rds"))
write.csv(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_IPCC_ref_regions_trends_by_product.csv"))
write.csv(ipcc_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_IPCC_ref_regions_problem_area_fraction.csv"))
write.csv(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_IPCC_ref_regions_problem_aggregated.csv"))
...
### save data ----
saveRDS(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_evap_quantiles_trends_by_product.rds"))
saveRDS(evap_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_evap_quantiles_problem_area_fraction.rds"))
saveRDS(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_evap_quantiles_problem_aggregated.rds"))
write.csv(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_evap_quantiles_trends_by_product.csv"))
write.csv(evap_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_evap_quantiles_problem_area_fraction.csv"))
write.csv(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_evap_quantiles_problem_aggregated.csv"))
...
### save data ----
saveRDS(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_elevation_trends_by_product.rds"))
saveRDS(elev_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_elevation_problem_area_fraction.rds"))
saveRDS(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_elevation_problem_aggregated.rds"))

write.csv(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_elevation_trends_by_product.csv"))
write.csv(elev_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_elevation_problem_area_fraction.csv"))
write.csv(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_elevation_problem_aggregated.csv"))
...
### save data ----
saveRDS(data_trend, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_Koeppen_Geiger_beck_trends_by_product.rds"))
saveRDS(KG_trends, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_Koeppen_Geiger_beck_problem_area_fraction.rds"))
saveRDS(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_Koeppen_Geiger_beck_problem_aggregated.rds"))


## KG beck main class ----

data_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "KG_beck_1_trend_bootstrap.rds"))  
ensemble_trend <- readRDS(paste0(PATH_SAVE_EVAP_TREND, "KG_beck_1_ensemble_trend_bootstrap.rds"))  

ensemble_trend[, dataset := "ensemble"]

data_trend <- merge(data_trend, ensemble_trend, 
                    by = c("dataset", "KG_beck_1", "p", "slope", "lower", "upper"),  all = T)
...
data_trend_slopes <- data_trend_slopes[order(rank)]
data_trend[, dataset := factor(dataset, levels = data_trend_slopes$dataset)]
data_trend <- data_trend[!is.na(KG_beck_1)]

data_trend[KG_beck_1 %in% c("A"), climate := "Equatorial"]
data_trend[KG_beck_1 %in% c("B"), climate := "Arid"]
data_trend[KG_beck_1 %in% c("C"), climate := "Temperate"]
data_trend[KG_beck_1 %in% c("D"), climate := "Continental"]
data_trend[KG_beck_1 %in% c("E"), climate := "Polar"]


### uncertainty for entire region ----

data_trend_env <- data_trend[dataset != "ensemble",.(Q25 = quantile(slope, 0.25),
                                                     Q75 = quantile(slope, 0.75)), KG_beck_1]
data_trend_env[, fold := abs(Q75)/abs(Q25)]
data_trend_env[abs(Q25) > abs(Q75), fold := abs(Q25)/abs(Q75)]
data_trend_env[Q75/Q25 < 0, sign := "different"]
data_trend_env[Q75/Q25 >= 0, sign := "same"]
...
data_trend_env[KG_beck_1 %in% c("A"), climate := "Equatorial"]
data_trend_env[KG_beck_1 %in% c("B"), climate := "Arid"]
data_trend_env[KG_beck_1 %in% c("C"), climate := "Temperate"]
data_trend_env[KG_beck_1 %in% c("D"), climate := "Continental"]
data_trend_env[KG_beck_1 %in% c("E"), climate := "Polar"]

### problems  ----
evap_trend_masks[, KG_beck_1 := strsplit(as.character(KG_beck), "")[[1]][1], .(lat, lon)]
KG_trends <- evap_trend_masks[,.(problem_area = sum(area)),.(problem, KG_beck_1)]
KG_trends <- KG_trends[complete.cases(KG_trends)]
KG_trends[, KG_area:= sum(problem_area), .(KG_beck_1)]
KG_trends[, KG_fraction:= problem_area/KG_area]
KG_trends[KG_beck_1 %in% c("A"), climate := "Equatorial"]
KG_trends[KG_beck_1 %in% c("B"), climate := "Arid"]
KG_trends[KG_beck_1 %in% c("C"), climate := "Temperate"]
KG_trends[KG_beck_1 %in% c("D"), climate := "Continental"]
...
saveRDS(data_trend_env, paste0(PATH_SAVE_EVAP_TREND_TABLES, "data_fig_Koeppen_Geiger_1_beck_problem_aggregated.rds"))
