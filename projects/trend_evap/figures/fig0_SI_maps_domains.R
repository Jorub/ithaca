...
## Koeppen-Geiger ----
evap_mask[, KG_beck_v2:= as.factor(KG_beck_v2)]

to_plot_sf <- evap_mask[, .(lon, lat, KG_beck_v2)
][, value := as.numeric(KG_beck_v2)]

mask_to_val <- unique(to_plot_sf[,(.(KG_beck_v2 = KG_beck_v2, value = value))])
...
fig_KG <- ggplot(to_plot_sf) +
  geom_sf(data = world_sf, fill = "light gray", color = "light gray") +
  geom_sf(aes(color = KG_beck_v2, fill = KG_beck_v2), lwd = 0.1) +
  geom_sf(data = earth_box, fill = NA, color = "black", lwd = 0.1) +
...
ggsave(paste0(PATH_SAVE_EVAP_TREND_FIGURES_SUPP,
              "fig_SI_KG_v2.png"), width = 8, height = 5)
